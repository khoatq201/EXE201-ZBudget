...
class _ChallengeScreenState extends State<ChallengeScreen> 
                          ...
                          child: Transform.rotate(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                color: AppColors.textInverse.withOpacity(0.08),
                              ),
                            ),
                          ),
                        ),
  ...
  Widget _buildStatsRow() {
    return Consumer<ChallengeService>(
      builder: (context, service, child) {
        final active = service.challenges.where((c) => c.status == ChallengeStatus.active).length;
        final completed = service.challenges.where((c) => c.status == ChallengeStatus.completed).length;
        ...
        return Row(
          children: [
            Expanded(child: _buildStatCard('Đang tham gia', '$active', Icons.play_circle_fill)),
            const SizedBox(width: AppSpacing.md),
            Expanded(child: _buildStatCard('Hoàn thành', '$completed', Icons.check_circle)),
            const SizedBox(width: AppSpacing.md),
            Expanded(child: _buildStatCard('Điểm tích lũy', '$totalPoints', Icons.star)),
          ],
        );
      },
    );
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Material(
                          color: Colors.transparent,
                          child: InkWell(
                            onTap: () => _navigateToDetail(context, challenge.id),
                            borderRadius: BorderRadius.circular(20),
                            child: Container(
                              padding: const EdgeInsets.symmetric(
                                horizontal: AppSpacing.lg,
  ...
  void _showStatsDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Tính năng thống kê chi tiết sẽ được cập nhật sớm.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Đóng'),
  ...
  void _showLeaderboardDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('So sánh thành tích của bạn với những người dùng khác.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Đóng'),
