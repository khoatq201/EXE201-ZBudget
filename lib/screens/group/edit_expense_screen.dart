...
class _EditExpenseScreenState extends State<EditExpenseScreen> {
  ...
  Future<void> _updateExpense() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Chi tiêu đã được cập nhật!')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteExpense() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Bạn có chắc chắn muốn xóa chi tiêu này không?'),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      try {
        ...
        if (mounted) {
          Navigator.pop(context);
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Chi tiêu đã được xóa!')),
          );
        }
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  // Reuse methods from AddExpenseScreen
